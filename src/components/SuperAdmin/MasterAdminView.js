...
const MasterAdminView = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://13.61.104.53:5000/api/var/superAdmin/getMasterAdmin/${masterCode}`,
      requestOptions
    )
      .then((response) => {
        if (!response.ok) {
  ...
  const handleStatusUpdate = async (status) => {
    if (selectedClient) {
      ...
      try {
        await axios.put(
          `http://13.61.104.53:5000/api/var/superAdmin/clients/${selectedClient._id}/status`,
          data,
          {
            headers: {
              "Content-Type": "application/json",
        ...
        <div className="bg-white p-8 rounded shadow-md w-full max-w-screen-xll mt-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white p-4 shadow rounded-lg">
              ...
              <div className="text-gray-700">
                ₹{masterAdminData.availableBudget}
              </div>
            </div>
            <div className="bg-white p-4 shadow rounded-lg">
