...
const WeeklyPnLPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      const config = {
        ...
        url: `http://16.16.64.168:5000/api/var/pnl/master-admin/${masterAdminId}/weekly-pnl`,
        headers: {
  ...
  if (error) {
    return <div className="text-red-500">{error.message || error}</div>;
  }
...
<Navbar/>
    <div className="container mx-auto p-6 bg-white text-blue-900 mt-16">
      <h1 className="text-2xl font-bold mb-4 border-b-2 border-blue-900">Weekly PnL</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {data.data.weeklyPnL.map((pnlItem, index) => (
          <div key={index} className="border border-blue-900 p-4 rounded shadow-md">
      ...
      <div className="mt-4 text-blue-900">
        <p><strong>All Clients Total Amount:</strong> ₹{data.data.AllClientTotalAmount}</p>
        <p><strong>All Clients Total Brokerage:</strong> ₹{data.data.AllClientTotalBrokerage}</p>
      </div>
