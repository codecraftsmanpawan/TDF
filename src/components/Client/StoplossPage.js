...
const Stoploss = () => {
    ...
    const handleUpdateStoploss = async (updatedStoploss) => {
        try {
            const token = localStorage.getItem('StocksUsertoken');
            await axios.put(`http://16.16.64.168:5000/api/var/client/stoploss/${selectedStoploss._id}`, updatedStoploss, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            setStoplosses(prev => prev.map(s => (s._id === selectedStoploss._id ? { ...s, ...updatedStoploss } : s)));
    ...
    const handleDeleteStoploss = async () => {
        try {
            const token = localStorage.getItem('StocksUsertoken');
            await axios.delete(`http://16.16.64.168:5000/api/var/client/stoploss/${selectedStoploss._id}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            setStoplosses(prev => prev.filter(s => s._id !== selectedStoploss._id));
