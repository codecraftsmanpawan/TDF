...
const Bids = () => {
    ...
    const confirmDelete = async () => {
        try {
            await axios.delete(`http://16.16.64.168:5000/api/var/client/bids/${selectedBid._id}`, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('StocksUsertoken')}` }
            });
            toast.success('Bid deleted successfully!'); // Show success toast
            setBids(prevBids => prevBids.filter(bid => bid._id !== selectedBid._id)); // Update bids state
        } catch (error) {
            setError(error);
    ...
    const updateBid = async (updatedBid) => {
        try {
            const response = await axios.put(`http://16.16.64.168:5000/api/var/client/bids/${selectedBid._id}`, updatedBid, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('StocksUsertoken')}` }
            });
            toast.success('Bid updated successfully!'); 
            setBids(prevBids => prevBids.map(bid => (bid._id === selectedBid._id ? response.data : bid))); // Update bids state
        } catch (error) {
            setError(error);
                        ...
                        bids.map((bid, index) => {
                            ...
                            return (
                                <tr key={bid._id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-blue-100'} ${backgroundColor} text-center`}>
                                    ...
                                    <td className={`py-2 px-4 border-b ${priceColor}`}>{bid.bidPrice ? `₹${bid.bidPrice}` : 'N/A'}</td>
